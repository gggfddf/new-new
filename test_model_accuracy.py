"""
Test Model Accuracy with Fresh Data
Train model on BTC data, save it, then test with fresh yfinance data
"""
...
def train_and_save_model():
    """Train model on BTC data and save it."""
    print("\n🔧 Training model on BTC data...")
    ...
    try:
        ...
        print(f"      Classifier accuracy: {model_results.performance_metrics['classifier_accuracy']:.3f}")
        print(f"      Target regressor R²: {model_results.performance_metrics['target_r2']:.3f}")
        print(f"      Duration regressor R²: {model_results.performance_metrics['duration_r2']:.3f}")
        
        return system, model_results
        
    except Exception as e:
        print(f"   ❌ Training failed: {e}")
        return None, None

def test_model_on_fresh_data(system, model_results):
    """Test trained model on fresh yfinance data."""
    print("\n🧪 Testing model on fresh yfinance data...")
    ...
    # Use last 1000 bars for testing (if available)
    ...
    try:
        ...
        print("   🔮 Generating predictions on fresh data...")
        ...
        for i, pred in enumerate(predictions[:5]):
            ...
            print(f"      Context: {context_tags[:3]}...")  # Show first 3 tags
...
def main():
    ...
    print("🚀 Model Accuracy Test with Fresh Data")
    print("=" * 80)
    print("🎯 Train on BTC data, save model, test on fresh yfinance data")
    print("=" * 80)
    
    # Step 1: Train and save model
    system, model_results = train_and_save_model()
    if system is None:
        print("❌ Cannot proceed - model training failed")
        return
    
    # Step 2: Test on fresh data
    predictions = test_model_on_fresh_data(system, model_results)
    ...
    if predictions:
        ...
        print(f"\n⚠️ PARTIAL SUCCESS")
        print(f"   ✅ Model trained and saved successfully")
        print(f"   ❌ Could not test on fresh data (no data or error)")
